...
export default function Layout({ children }: LayoutProps) {
  ...
  const [advertiseFlow, setAdvertiseFlow] = useState<string | null>(null);
  ...
  const isAmbassadorPage = location === "/ambassador";
  const isAdvertiseOpen = advertiseFlow === "advertise";
    ...
    <div className="h-screen w-full flex flex-col bg-black overflow-hidden">
      ...
      {!showSeasonEnd && (
        <div className="fixed bottom-6 left-0 right-0 z-50 flex items-center justify-center gap-3 px-4">
          ...
          {/* ── Ambassador page: only a back button ── */}
          {isAmbassadorPage ? (
            <motion.button
              onClick={() => navigate("/")}
              whileTap={{ scale: 0.88 }}
              ...
              style={{
                ...
                border: "none", boxShadow: "0 8px 24px rgba(0,0,0,0.45)",
              }}
            >
              <ChevronLeft style={{ width: 24, height: 24, color: "rgba(255,255,255,0.9)", strokeWidth: 2.2 }} />
            </motion.button>
            ...
            <>
              ...
              <nav className="flex justify-center items-center h-16 bg-[#1C1C1E]/90 backdrop-blur-md rounded-[40px] shadow-2xl px-7 gap-8">
                ...
                <motion.button
                  ...
                  className={`flex flex-col items-center justify-center gap-1 ${
                    isHomeActive || location.startsWith("/admin") ? "text-white" : "text-[#6E6E73]"
                  }`}
                >
                  <div
                  ...
                  return (
                    <Link key={item.href} href={item.href}>
                      <motion.button
                        whileTap={{ scale: 0.82 }}
                        transition={{ type: "spring", stiffness: 400, damping: 17 }}
                        className={`flex flex-col items-center justify-center gap-1 ${isActive ? "text-white" : "text-[#6E6E73]"}`}
                      >
                        <Icon className="w-8 h-8 transition-all" strokeWidth={isActive ? 2.5 : 2} />
                        <span className="text-[9px] font-medium leading-none tracking-wide"
              ...
              {/* ── Floating circle: + normally · ← when advertise sheet is open ── */}
              <motion.button
                onClick={isAdvertiseOpen ? () => setPanelOpen(false) : handlePlusClick}
                whileTap={{ scale: 0.88 }}
                whileHover={{ scale: 1.06 }}
                animate={{ rotate: panelOpen && !isAdvertiseOpen ? 45 : 0 }}
                transition={{ type: "spring", stiffness: 380, damping: 22 }}
                className="flex items-center justify-center rounded-full flex-shrink-0"
                style={{
                  width: "56px", height: "56px",
                  background: panelOpen && !isAdvertiseOpen
                    ? "rgba(255,255,255,0.14)"
                    : "rgba(28,28,30,0.9)",
                  backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)",
                  border: panelOpen && !isAdvertiseOpen
                    ? "1px solid rgba(255,255,255,0.18)"
                    : "none",
                  boxShadow: panelOpen && !isAdvertiseOpen
                    ? "0 0 0 6px rgba(255,255,255,0.05)"
                    : "0 8px 24px rgba(0,0,0,0.45)",
                }}
                aria-label={isAdvertiseOpen ? "Back" : "Create"}
              >
                {isAdvertiseOpen
                  ? <ChevronLeft style={{ width: 24, height: 24, color: "rgba(255,255,255,0.9)", strokeWidth: 2.2 }} />
                  : <Plus style={{ width: 24, height: 24, color: "rgba(255,255,255,0.9)", strokeWidth: 2.2 }} />
                }
              </motion.button>
            </>
          )}
      ...
      <CreatePanel open={panelOpen} onClose={() => setPanelOpen(false)} onFlowChange={setAdvertiseFlow} />
    </div>
  );
}
