...
export function useAuth() {
  ...
  const telegramAuthMutation = useMutation({
    ...
    onSuccess: (userData) => {
      ...
      // NOTE: Do NOT call setQueryData here.
      // The /api/auth/telegram endpoint returns raw SQL snake_case keys (usd_balance, ton_balance, etc.)
      // but the client expects camelCase (usdBalance, tonBalance, etc.).
      // Setting cache directly with snake_case data causes all balance fields to read as undefined → shows $0.
      // The GET /api/auth/user (via Drizzle ORM) always returns correct camelCase and is already in flight.
      // Just cache locally for offline use and let the GET query update the React Query cache.
